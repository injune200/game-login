...
router.post("/item-create", async (req, res, next) => { // 아이템 생성
    try {
        const { itemName,stats,itemPrice } = req.body;

        const isExisUser = await prismaItem.items.findFirst({ //중복 여부 조회
            where: { itemName }
        })
        ...
        const item = await prismaItem.items.create({ //변수 저장
            data: {
                itemName,
                stats: {
                    create:{
...
router.get("/item-list", async (req, res, next) => { // 아이템 목록 조회
    try {
...
router.put("/item-update/:itemName", async (req, res, next) => { // 아이템 수정
    try {
...
router.get("/item-list/:itemName", async (req, res, next) => { // 아이템 상세 조회
    try {
...
router.delete("/item-delete/:itemName", async (req, res, next) => {//아이템 삭제
    try {
        const { itemName } = req.params;
        const item = await prismaItem.items.findFirst({
            where: { itemName }
