...
router.post('/sign-up', async (req, res, next) => {
  ...
  try {
    ...
    const isExisUser = await prismaUser.users.findFirst({
      //중복 여부 조회
      where: { userId },
    });
    ...
    const user = await prismaUser.users.create({
      //변수 저장
      data: {
        userId,
...
router.delete('/sign-delete', authMiddleware, async (req, res, next) => {
  //회원 삭제
  try {
